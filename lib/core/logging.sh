...
${__BENCHMARK_SOURCE_LIB_CORE_LOGGING_SH__:=false} && return 0 || __BENCHMARK_SOURCE_LIB_CORE_LOGGING_SH__=true

# Bash version check: must be >= 4 to has associated array supported!
if ((BASH_VERSINFO[0] < 4)) && ((BASH_VERSINFO[1] < 4)); then
  echo >&2 "Require bash version >= 4.4, current version is ${BASH_VERSION}!"
  return 1
fi
...
function logging::error() {
  if logging::_is_level_enabled _LOGGING_LEVEL_ERROR; then
    if [[ "${_LOGGING_ERROR_TO_STDERR}" == true ]]; then
      echo >&2 "$(color::ansi::red "[ERROR]")$(logging::_additional_tags)" "$@"
    else
      echo "$(color::ansi::red "[ERROR]")$(logging::_additional_tags)" "$@"
    fi
  fi
}
...
function logging::errorf() {
  ...
  if logging::_is_level_enabled _LOGGING_LEVEL_ERROR; then
    if [[ "${_LOGGING_ERROR_TO_STDERR}" == true ]]; then
      printf >&2 "$(color::ansi::red "[ERROR]")$(logging::_additional_tags) $1" "${@:2}"
    else
      printf "$(color::ansi::red "[ERROR]")$(logging::_additional_tags) $1" "${@:2}"
    fi
  fi
}
...
function logging::debug() {
  if logging::_is_level_enabled _LOGGING_LEVEL_DEBUG; then
    echo >&2 "[DEBUG]$(logging::_additional_tags)" "$@"
  fi
}
...
function logging::debugf() {
  # shellcheck disable=SC2059
  if logging::_is_level_enabled _LOGGING_LEVEL_DEBUG; then
    printf >&2 "[DEBUG]$(logging::_additional_tags) $1" "${@:2}"
  fi
}
