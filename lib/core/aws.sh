# Perform operations on AWS resources with the awscli.
${__BENCHMARK_SOURCE_LIB_CORE_AWS_SH__:=false} && return 0 || __BENCHMARK_SOURCE_LIB_CORE_AWS_SH__=true
...
function awscli::s3api::delete_bucket() {
  ...
  if [[ "${AWSCLI_S3API_FORCE_DELETE_BUCKET}" == "true" ]]; then
    common::run awscli::aws s3 rm s3://"${bucket}" --recursive --quiet
...
#######################################
# Delete a bucket folder in the specified region.
# Globals:
#   AWSCLI_PROFILE
# Arguments:
#   Bucket folder name.
# Returns
#   0 if the bucket folder is deleted, non-zero on error.
#######################################
function awscli::s3api::delete_bucket_folder() {
  (($# == 1)) || { echo >&2 "not enough arguments" && return 1; }
  [[ -n "$1" ]] || { echo >&2 "bucket must be provided" && return 1; }

  local bucket_folder=$1

  common::run awscli::aws s3 rm s3://"${bucket_folder}" --recursive --quiet
}